...
脆弱到 case_reorder2.hs 中有點錯誤就完全無法 debug 。 DeCaseReorder.hs 中由 TH
生出來的部分要講清楚 l 是 SrcInfo 有些不用是怎樣？ 有 fmap 的部分取代起來很麻煩
。腦中沒有作者的 context ，越看越花。

讀過就好了嗎？不能好好說明 `altToPartial`, `dataConToShape`, `buildReorder`,
`orderedCaseToExp` 的話就沒有意義。


2016-02-23 406d3be
==================

中間斷個幾天，幾乎得重看。 rhsToExp 仍需要拿個例子來追看看。

DeCaseReorder 由 template 生出的部分太不一樣了。真的都是靠取代做的嗎？根本不知道
怎麼重現！只能一點一點改成相同的樣子。
