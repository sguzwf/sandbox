...
moveMoving (Reversing [] bs hs) = moveMoving (Concating cs cs) where -- 註：被前面拉住這件事，在 report 中有嗎？
  cs = hs ++ bs
moveMoving (Concating cs (h:hs)) = Concating cs hs
moveMoving (Concating cs []) = Concating cs [] -- 註：傳說中的平靜模式 XD
